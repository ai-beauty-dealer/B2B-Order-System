...
- パスワードは全サロン共通(`actim`)。完全自動ログインだと共用端末で誰でもそのサロンに入れる
- **対策**: 「続ける」方式にする。起動時に **「◯◯サロンとして続ける」ボタン＋「別のサロンでログイン」** の2択。手数は自動ログインとほぼ同じで、切り替えも安全。ログイン結果（clientName/clientType）をlocalStorageに保存して再開に使う
